...
- `updateTable()` copied rows from a pre-module migration table with an empty `module`, which `List()` filters out, so every historical migration re-ran after the upgrade; the rows are now copied as the `base` module. An installation already upgraded by an earlier version needs `module` set to `base` by hand, since the table engine (`TinyLog`) does not support updates.
