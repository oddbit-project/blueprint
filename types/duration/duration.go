...
// glance.
//
// The canonical type is duration.Duration (Go-idiomatic stutter, like
// time.Time or url.URL). Construct values with the named helpers,
// which all return Duration:
//
...
// Duration is a duration measured in whole seconds. The default
// encoding/json codec serializes it as a plain integer (e.g. 900 for
// fifteen minutes) — no custom marshaler is required because Duration
...
// Seconds returns the duration equal to n seconds.
func Seconds(n int64) Duration { return Duration(n) }

// Minutes returns the duration equal to n minutes.
func Minutes(n int64) Duration { return Duration(n * 60) }

// Hours returns the duration equal to n hours.
func Hours(n int64) Duration { return Duration(n * 3600) }
...
func Days(n int64) Duration { return Duration(n * 86400) }

// FromStd converts a time.Duration to Duration, truncating toward
